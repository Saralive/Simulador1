...
from reglas import matrices, frecuencia, nodos_seleccionados
...
class ComplexNetwork(Model):
    ...
    def __init__(self,main,numEnlacesDinamicos,maximasConexiones,num_paquetes,algoritmoEncaminamiento,regla,vectorPopularidad,vectorDistancia,alfa):#constructor de la clase ComplexNetwork
        self.__main=main
        ...
        self.__regla=regla
        # Parámetros de la regla 4
        self.__vectorPopularidad = vectorPopularidad
        self.__vectorDistancia = vectorDistancia
        self.__alfa = alfa
    ...
    def init(self):
        ...
        self.matriz_rutas = [] # Matriz de rutas 
        self.long = len(self.__vectorDistancia) #Longitud máxima de las rutas de exploración
    ...
    def receive(self, event):
            ...
            if(len(event.package.ruta) > 0):#si todavía falta recorrer nodos en la ruta    
                ...
                for enlace in self.enlacesDinamicos:
                    if(event.source==enlace.idConectado):
                        ...
                        break
                # ESTE BLOQUE SE DEBE EJECUTAR PARA LA COMBINACION DE LA REGLAS
                if (self.__regla == 4):
                    self.matriz_rutas.append(event.package.rutaAuxiliar[2:]) #guarda la ruta del nodo a distancia en saltos igual a 2 en adelante
                # ESTE BLOQUE SE DEBE EJECUTAR PARA LAS REGLAS 1,2 Y 3: ACTUALIZA f_n
                else:
        ...
        #PIF fase de NEGOCIACION--------------------------------
            ...
            if(self.visited==False):
                ...
                if(self.__regla == 4):
                    popularidad, distancia = matrices(self.long, self.matriz_rutas)#crea las matrices de popularidad y distancia
                    self.f_n = nodos_seleccionados(popularidad, distancia, self.__vectorPopularidad, self.__vectorDistancia, self.__alfa)
                ...
                if(len(self.f_n)>0):#si existen candidatos a reconexión en f_n:
                    for enlace in self.enlacesDinamicos:#recorro mi lista de enlaces dinámicos en busca de enlaces libres
                        if(enlace.libre==True):#si el enlace en cuestión está libre, mando solicitudes de CABLEADO
                            ...
                            if(self.__regla==3 or self.__regla==4):#si trabajo con R3 o R4:
                    ...
                    if(not(hayEnlacesLibres)):#si no hay enlaces libres:
                        ...
                        if(self.f_e[clavesF_e[0]]<self.frecEnlace):#si la frecuencia de uso del enlace es menor que el umbral self.frecEnlace, entonces busco el recableado
                            ...
                            if(self.__regla==3 or self.__regla==4):#si trabajo con R3:
                                llaves=list(self.f_n.keys())
                                probabilidades=list(self.f_n.values())
                                probabilidades=probabilidades/np.sum(probabilidades)
                                #print("soy",self.id,"ejecuto R3 mis llaves son:",llaves)
